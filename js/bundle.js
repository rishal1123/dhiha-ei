...
(function() {
    ...
    // ============================================
    // MULTIPLAYER MODULE WRAPPER
    // Uses shared js/multiplayer.js module
    // ============================================

    // Wrapper functions that delegate to shared Multiplayer module
    function initializeMultiplayer() {
        return Multiplayer.init();
    }

    function isMultiplayerAvailable() {
        return Multiplayer.isAvailable();
    }

    // Helper to get socket for direct operations (matchmaking etc.)
    function getSocket() {
        return Multiplayer.getSocket();
    }

    // For compatibility - code uses 'socket' directly for matchmaking operations
    // We define socket as a proxy object that delegates to the Multiplayer module's socket
    const socket = new Proxy({}, {
        get(target, prop) {
            const realSocket = Multiplayer.getSocket();
            if (!realSocket) return undefined;
            const value = realSocket[prop];
            // Bind methods to the real socket
    ...
    // Also expose currentUserId getter
    function getCurrentUserId() {
        return Multiplayer.getCurrentUserId();
    }

    // Get classes from shared module
    const { PresenceManager, LobbyManager, GameSyncManager, DiGuLobbyManager, DiGuSyncManager } = Multiplayer;
